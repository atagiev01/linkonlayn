...
export default function App() {
  ...
  useEffect(() => {
    function resolveRoute() {
      ...
      // Admin panel is reachable ONLY via /admin-01 — the old /admin path,
      // #admin hash, and ?admin=true query param are intentionally no longer
      // accepted, so the panel isn't discoverable at the obvious /admin URL.
      if (path.startsWith('/admin-01')) {
        setActivePublicSlug(null);
        setPreviewTemplateId(null);
  ...
  const handleReturnToAdmin = () => {
    ...
    setIsPreviewMode(false);
    setAppView('admin');
    window.history.pushState({}, '', '/admin-01');
  };
