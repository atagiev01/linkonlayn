  ...
  useEffect(() => {
    ...
    const tryPlay = () => {
      ...
      audio
        ...
        .catch(() => {
          // Brauzer avtomatik səsləndirməni bloklayıb; ilk toxunuşu gözləyirik
        });
    };

    // Brauzer icazə verirsə, səhifə açılan kimi başlasın
    tryPlay();

    // Əks halda ilk toxunuş / klik / düymə / sürüşdürmədə başlasın
    const events: (keyof WindowEventMap)[] = ['pointerup', 'click', 'touchend', 'keydown'];
    const removeListeners = () => events.forEach((e) => window.removeEventListener(e, tryPlay));
    events.forEach((e) => window.addEventListener(e, tryPlay));

    return removeListeners;
  }, [isMusicValid, musicUrl]);
