...
// Firestore-a qoşulma problemi (yanlış config, şəbəkə, offline və s.) olduqda
// `await`-i sonsuza kimi asılı saxlamamaq üçün: müəyyən vaxtdan sonra fallback
// dəyərlə "uğur" kimi davam edirik ki, səhifə əbədi "Yüklənir..." vəziyyətində
// qalmasın.
...
export const PublicInvitationView: React.FC<PublicInvitationViewProps> = ({
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        const allTemplates = await withTimeout(fetchTemplates(), 6000, DEFAULT_TEMPLATES);
        if (!isMounted) return;
  ...
  // Dəvətnamənin öz musiqisi heç təyin olunmayıbsa, şablonun standart musiqisi çalınır
  const effectiveInvitation: Invitation =
    invitation.music == null && invitation.musicEnabled !== false && activeTpl.music
      ? { ...invitation, music: activeTpl.music, musicTitle: invitation.musicTitle || activeTpl.musicTitle }
      : invitation;

  // Admin şablon üçün şrift seçibsə, bütün .font-serif / .font-sans elementlərinə tətbiq olunur
  const fontCss = activeTpl.themeConfig?.fontsCustomized
    ? `${
        activeTpl.themeConfig.fontFamilyHeading
          ? `.tpl-fonts .font-serif{font-family:${activeTpl.themeConfig.fontFamilyHeading} !important}`
          : ''
      }${
        activeTpl.themeConfig.fontFamilyBody
          ? `.tpl-fonts .font-sans{font-family:${activeTpl.themeConfig.fontFamilyBody} !important}`
          : ''
      }`
    : '';

  return (
    <div className="relative min-h-screen bg-stone-950 tpl-fonts">
