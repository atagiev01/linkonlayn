...
export const InvitationEditorModal: React.FC<InvitationEditorModalProps> = ({
  ...
  const [celebrantAge, setCelebrantAge] = useState(initialData?.celebrantAge || '');
  const [cardIntro, setCardIntro] = useState(initialData?.cardIntro || '');
  const [cardTitle, setCardTitle] = useState(initialData?.cardTitle || '');
  const [cardReceptionNote, setCardReceptionNote] = useState(initialData?.cardReceptionNote || '');
  // Yeni dəvətnamədə seçilmiş şablonun standart musiqisi başlanğıc dəyər olur
  const initialTemplate = templates.find((t) => t.id === (initialData?.templateId || templates[0]?.id));
  const [music, setMusic] = useState(
    initialData?.music && initialData.music !== 'none'
      ? initialData.music
      : initialTemplate?.music || WEDDING_MUSIC_PRESETS[0].url
  );
  const [musicTitle, setMusicTitle] = useState(
    initialData?.musicTitle ||
      (initialTemplate?.music ? initialTemplate.musicTitle || '' : WEDDING_MUSIC_PRESETS[0].defaultTitle)
  );
  // Admin musiqini əl ilə dəyişibsə, şablon dəyişəndə üzərinə yazılmasın
  const [musicTouched, setMusicTouched] = useState<boolean>(!!initialData);
  ...
  const [musicMode, setMusicMode] = useState<'preset' | 'custom'>(
    (() => {
      const current = initialData?.music || initialTemplate?.music;
      return current && !WEDDING_MUSIC_PRESETS.some((p) => p.url === current) ? 'custom' : 'preset';
    })()
  );
  ...
  const handleSelectTemplate = (tpl: Template) => {
    ...
    if (!musicTouched && tpl.music) {
      setMusic(tpl.music);
      setMusicTitle(tpl.musicTitle || '');
      setMusicMode(WEDDING_MUSIC_PRESETS.some((p) => p.url === tpl.music) ? 'preset' : 'custom');
  ...
  const [contactPhone, setContactPhone] = useState(initialData?.contactPhone || '+994 55 651 93 90');
  ...
  const isBirthday = templates.find((t) => t.id === templateId)?.eventType === 'birthday';
  const hasCardTexts = templateId === 'editorial-press-card' || templateId === 'birthday-card';
  ...
  const handleGenerateSlug = () => {
    ...
    const cleanBride = brideName
      ...
      .replace(/ş/g, 's')
      .replace(/[^a-z0-9]/g, '');
    setSlug(isBirthday ? `${cleanGroom}-ad-gunu` : `${cleanGroom}-ve-${cleanBride}`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!groomName.trim() || (!isBirthday && !brideName.trim())) {
      setError(isBirthday ? 'Ad günü sahibinin adı mütləq daxil edilməlidir.' : 'Gəlin və Bəyin adları mütləq daxil edilməlidir.');
      return;
    }
    if (!venue.trim()) {
    ...
    try {
      const inv: Invitation = {
        ...
        celebrantAge: isBirthday ? celebrantAge.trim() : '',
        brideName: isBirthday ? '' : brideName.trim(),
        groomName: groomName.trim(),
        brideParents: isBirthday ? '' : brideParents.trim(),
        groomParents: isBirthday ? '' : groomParents.trim(),
        weddingDate,
        weddingTime,
        venue: venue.trim(),
        ...
        cardIntro: cardIntro.trim(),
        cardTitle: cardTitle.trim(),
        cardReceptionNote: cardReceptionNote.trim(),
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4 pt-4 border-t border-stone-800">
            <label className="block text-xs uppercase tracking-wider font-semibold text-amber-400">
              {isBirthday ? '2. Ad Günü Sahibi' : '2. Bəy & Gəlin Məlumatları'}
            </label>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-stone-400 mb-1">{isBirthday ? 'Ad günü sahibinin adı *' : 'Bəyin Adı *'}</label>
                <input
                  type="text"
                  required
                  value={groomName}
              ...
              {isBirthday ? (
                <div>
                  <label className="block text-xs text-stone-400 mb-1">Neçə yaşı tamam olur? (İstəyə görə)</label>
                  <input
                    type="text"
                    inputMode="numeric"
                    value={celebrantAge}
          ...
          <div className="space-y-4 pt-4 border-t border-stone-800">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs text-stone-400 mb-1">{isBirthday ? 'Tarix *' : 'Toy Tarixi *'}</label>
                <input
                  type="date"
                  required
                  value={weddingDate}
            ...
            {hasCardTexts && (
              <div className="space-y-3 p-4 rounded-2xl bg-stone-950 border border-amber-500/30">
                <p className="text-[11px] uppercase tracking-wider font-semibold text-amber-400">
                  Kart mətnləri
                  <span className="text-stone-500 normal-case font-normal"> (boş qalsa standart mətn göstərilir)</span>
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs text-stone-400 mb-1">Yuxarı sol mətn</label>
                    <input
                      type="text"
                      value={cardIntro}
                      onChange={(e) => setCardIntro(e.target.value)}
                      placeholder={isBirthday ? 'Sizi ad günümə dəvət edirik' : 'Sizi toyumuza dəvət edirik'}
                      className="w-full px-4 py-2 rounded-xl bg-stone-900 border border-stone-800 text-stone-100 text-xs focus:border-amber-400 focus:outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-stone-400 mb-1">Böyük başlıq (hər sətir ayrıca sətir, maks. 4)</label>
                    <textarea
                      rows={3}
                      value={cardTitle}
                      onChange={(e) => setCardTitle(e.target.value)}
                      placeholder={isBirthday ? 'AD\nGÜNÜ' : 'TOYUMUZA\nDƏVƏT'}
                      className="w-full p-3 rounded-xl bg-stone-900 border border-stone-800 text-stone-100 text-xs focus:border-amber-400 focus:outline-none resize-none"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-stone-400 mb-1">Şaquli zolaqdakı son qeyd</label>
                    <input
                      type="text"
                      value={cardReceptionNote}
                      onChange={(e) => setCardReceptionNote(e.target.value)}
                      placeholder={isBirthday ? 'Tort və əyləncə' : 'Ziyafət davam edəcək'}
                      className="w-full px-4 py-2 rounded-xl bg-stone-900 border border-stone-800 text-stone-100 text-xs focus:border-amber-400 focus:outline-none"
                    />
                    <p className="text-[10px] text-stone-500 mt-1">
                      Zolaq: saat | ünvan, şəhər | bu qeyd — saat və ünvan yuxarıdakı sahələrdən avtomatik götürülür.
                    </p>
                  </div>
                </div>
              </div>
